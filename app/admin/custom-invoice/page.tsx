...
export default function BranchAdminInvoices() {
  ...
  // 🔥 Firebase se real-time invoices fetch - branch specific
  useEffect(() => {
  ...
  // 🔥 Fetch Services, Branch, Categories, Staff - branch specific
  useEffect(() => {
    const unsubServices = onSnapshot(
      query(collection(db, 'services'), where('branchNames', 'array-contains', user?.branchName || '')),
      (snap) => {
    ...
    const unsubBranches = onSnapshot(
      query(collection(db, 'branches'), where('id', '==', user?.branchId || '')),
      (snap) => {
        if (!snap.empty) {
          const branchData = {
            id: snap.docs[0].id,
    ...
    const unsubCategories = onSnapshot(
      query(collection(db, 'categories'), where('branchName', '==', user?.branchName || '')),
      (snap) => {
    ...
    const unsubStaff = onSnapshot(
      query(collection(db, 'staff'), where('branch', '==', user?.branchName || '')),
      (snap) => {
  ...
  const saveInvoice = async () => {
    try {
      ...
      // Create invoice data with branch filter
      const invoiceData = {
        ...invoiceForm,
        branch: user?.branchName || invoiceForm.branch,
